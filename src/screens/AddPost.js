...
const baseUrl = 'https://socialmedia.mlmcreatorsindia.com/api/posts';
const token = 'cbUYSNd5ksuz0IgKlifpXV8mPL5P1jY4XuHlt977b9f0d783';

export default function AddPost({ navigation }) {
    // const [selectedImages, setSelectedImages] = useState([]);
    const [media, setMedia] = useState([]);
    const [caption, setCaption] = useState(""); // Changed from content to caption
    ...
    const handleImagePicker = async () => {
        try {
            ...
            // setMedia(images.map((image) => ({ uri: image.path })));
            setMedia([...media, ...images.map((image) => ({ uri: image.path }))]);
        } catch (error) {
    ...
    const handleCameraPicker = async () => {
        try {
            ...
            // setMedia([{ uri: image.path }]);
            setMedia([...media, { uri: image.path }]);
    ...
    const handlePost = async () => {
        ...
        try {
            ...
            formData.append('content', caption); // Changed from content to caption
            ...
            Alert.alert("Success", response.data.message); // Show success message from response
            // setSelectedImages([]);
            setMedia([]);
            setCaption("");
            console.log("Response:", response.data.post); // Log the created post data
            // Handle success response here
        } catch (error) {
            console.error("Error posting:", error);
            Alert.alert("Error", "Failed to post. Please try again."); // Show error message
            // Handle error here
        } finally {
            setIsDisabled(false);
        }
...
// const baseUrl = 'https://guflu.in/Social_media/addpost.php';
...
//                 compressImageQuality: 0.5,
//             });
//             setSelectedImages([...selectedImages, ...images.map((image) => ({ uri: image.path }))]);
//         } catch (error) {
//             console.error("Error selecting images:", error);
//             Alert.alert("Error", "Failed to select images. Please try again.");
//         }
...
//                 cropping: true,
//             });
//             setSelectedImages([...selectedImages, { uri: image.path }]);
//         } catch (error) {
//             console.error("Error opening camera:", error);
//             Alert.alert("Error", "Failed to open camera. Please try again.");
//         }
...
//             // Handle error here
//         } finally {
//             setIsDisabled(false); // Enable the button after the operation is complete
//         }
//     };
